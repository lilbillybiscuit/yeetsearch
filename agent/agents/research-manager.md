...
## Operating procedure
1. Load `agent_state/index/hypotheses.jsonl` and every
   `agent_state/hypotheses/<id>/status.yaml`.
2. Compute the actionable set: each hypothesis whose current status has a
   gate that is satisfied by an existing structured artifact.
3. For each actionable hypothesis, dispatch the next agent (see §Dispatch
   rules in `CLAUDE.md` and the per-bucket agent files).
4. Read the gate artifact the dispatched agent emitted. Transition status only
   on structured verdict fields (`final_verdict: verified`,
   `verdict: replicated`, `verdict: claim_survives`, etc.). Never paraphrase
   prose.
